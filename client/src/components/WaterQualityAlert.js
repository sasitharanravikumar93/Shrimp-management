...
const WaterQualityAlert = ({ 
  ...
  const checkOverallQuality = () => {
    ...
    const invalidParams = parameters.filter(p => 
      p.value !== undefined && p.value !== null && !isParameterOk(p.value, p.param)
    );
    ...
    >
      <Card variant="outlined" sx={{ height: '100%' }}>
        <CardContent>
          ...
          <Alert 
            severity={overallQuality === 'good' ? 'success' : overallQuality === 'fair' ? 'warning' : 'error'}
            sx={{ mb: 2 }}
          >
            <AlertTitle>
              {overallQuality === 'good' ? 'Good Water Quality' : 
               overallQuality === 'fair' ? 'Fair Water Quality' : 'Poor Water Quality'}
            </AlertTitle>
          ...
          {(pH !== undefined || dissolvedOxygen !== undefined || temperature !== undefined) && (
            <Box sx={{ mb: 1 }}>
              ...
              <Typography variant="body2">
                ...
                {dissolvedOxygen !== undefined && `DO: ${optimalRanges.do.min}-${optimalRanges.do.max} mg/L | `}
                {temperature !== undefined && `Temp: ${optimalRanges.temp.min}-${optimalRanges.temp.max}°C`}
              </Typography>
            </Box>
          )}
        </CardContent>
