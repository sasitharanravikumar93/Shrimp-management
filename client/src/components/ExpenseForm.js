...
const ExpenseForm = ({ open, onClose, onSave, expense }) => {
  ...
  const farmSubCategories = ['Electricity', 'Fuel', 'Maintenance', 'Security', 'Office Supplies', 'Transportation'];
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <Controller
                name="date"
                control={control}
                render={({ field }) => <TextField {...field} type="date" label="Date" fullWidth InputLabelProps={{ shrink: true }} />}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <Controller
                name="amount"
                control={control}
                render={({ field }) => <TextField {...field} type="number" label="Amount" fullWidth />}
              />
            </Grid>
            <Grid item xs={12}>
              <Controller
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField {...field} select label="Sub Category" fullWidth>
                      {cultureSubCategories.map(cat => <MenuItem key={cat} value={cat}>{cat}</MenuItem>)}
                    </TextField>
                  )}
                />
              </Grid>
            ...
            {mainCategory === 'Farm' && (
              <Grid item xs={12}>
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField {...field} select label="Sub Category" fullWidth>
                      {farmSubCategories.map(cat => <MenuItem key={cat} value={cat}>{cat}</MenuItem>)}
                    </TextField>
                  )}
                />
              </Grid>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField {...field} select label="Pond" fullWidth>
                      {ponds.data?.map(pond => <MenuItem key={pond._id} value={pond._id}>{pond.name}</MenuItem>)}
                    </TextField>
                  )}
                />
              </Grid>
