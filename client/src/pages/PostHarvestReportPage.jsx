...
const PostHarvestReportPage = () => {
  ...
  const avgFCR = harvests.length > 0 
    ? harvests.reduce((sum, h) => sum + (h.fcr || 0), 0) / harvests.filter(h => h.fcr).length 
    : 0;
  const avgSurvival = harvests.length > 0 
    ? harvests.reduce((sum, h) => sum + (h.survivalRate || 0), 0) / harvests.filter(h => h.survivalRate).length 
    : 0;
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4, '@media print': { mt: 0, p: 0, width: '100vw' } }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4, '@media print': { display: 'none' } }}>
        <Typography variant="h4" component="h1">Post-Harvest Report</Typography>
      ...
      <Card elevation={0} variant="outlined" sx={{ p: 4, '@media print': { border: 'none', p: 0 } }}>
        <Typography variant="h3" align="center" gutterBottom>Season Summary Report</Typography>
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={4}>
            <Card variant="outlined" sx={{ height: '100%', bgcolor: financials?.netProfitLoss >= 0 ? 'success.light' : 'error.light', color: 'white' }}>
              <CardContent>
                <Typography variant="h6">Net Profit/Loss</Typography>
                <Typography variant="h3">
                  ${Math.abs(financials?.netProfitLoss || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </Typography>
                <Typography variant="subtitle1">({financials?.netProfitLoss >= 0 ? '+' : '-'}{financials?.margin?.toFixed(2)}%)</Typography>
              </CardContent>
            </Card>
          </Grid>
          <Grid item xs={12} md={4}>
            <Card variant="outlined" sx={{ height: '100%', bgcolor: 'info.light', color: 'white' }}>
              <CardContent>
                ...
                <Typography variant="body1" fontSize={20}>Survival Rate: {avgSurvival.toFixed(2)}%</Typography>
                <Typography variant="body1" fontSize={20}>FCR: {avgFCR.toFixed(2)}</Typography>
              </CardContent>
            </Card>
          </Grid>
        </Grid>
        ...
        <Box sx={{ mt: 5 }}>
          <Typography variant="h5" gutterBottom>Financials Breakdown</Typography>
          <TableContainer>
            <Table size="small">
              <TableBody>
                <TableRow>
                  <TableCell><Typography variant="body1">Total Revenue from Sales</Typography></TableCell>
                  <TableCell align="right"><Typography variant="body1" color="success.main" fontWeight="bold">${(financials?.totalRevenue || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</Typography></TableCell>
                </TableRow>
                <TableRow>
                  <TableCell><Typography variant="body1">Total Operational Cost (OPEX)</Typography></TableCell>
                  <TableCell align="right"><Typography variant="body1" color="error.main" fontWeight="bold">-${(financials?.totalCost || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</Typography></TableCell>
                </TableRow>
                <TableRow>
                  <TableCell><Typography variant="h6">Net Margin</Typography></TableCell>
                  <TableCell align="right">
                    <Typography variant="h6" color={financials?.netProfitLoss >= 0 ? 'success.main' : 'error.main'}>
                      ${(financials?.netProfitLoss || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </Typography>
                  </TableCell>
                </TableRow>
              </TableBody>
        ...
        <Box sx={{ mt: 5 }}>
          <Typography variant="h5" gutterBottom>Pond-by-Pond Harvest KPIs</Typography>
        ...
        <Box sx={{ mt: 8, display: 'flex', justifyContent: 'space-between', '@media print': { pageBreakInside: 'avoid' } }}>
