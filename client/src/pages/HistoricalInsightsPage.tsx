...
const HistoricalInsightsPage: React.FC = () => {
  ...
  } = useHistoricalPondsBySeason(selectedSeason1);
  const {
    data: season2PondsData,
    loading: season2PondsLoading,
    error: season2PondsError
  } = useHistoricalPondsBySeason(selectedSeason2);
    ...
    if (mode === 'historical') {
        ...
        const day =
          Math.floor(
            (new Date(item.timestamp).getTime() -
              new Date(comparisonData.pond_a.season.startDate).getTime()) /
              (1000 * 3600 * 24)
          ) + 1;
        allDays.add(day);
      ...
      metricData.pond_b_data.forEach((item: any) => {
        const day =
          Math.floor(
            (new Date(item.timestamp).getTime() -
              new Date(comparisonData.pond_b.season.startDate).getTime()) /
              (1000 * 3600 * 24)
          ) + 1;
        allDays.add(day);
            ...
            <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        label={t('pond_a')}
                      >
                        {(currentSeasonPondsData as any)?.ponds?.map((p: any) => (
                ...
                <>
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        label={t('season_1')}
                      >
                        {(historicalSeasonsData as any)?.seasons?.map((s: any) => (
                  ...
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth>
                      ...
                      >
                        {(season1PondsData as any)?.ponds?.map((p: any) => (
                          <MenuItem key={p.id} value={p.id}>
                            {p.name}
                          </MenuItem>
                        ))}
                  ...
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth>
                      ...
                      >
                        {(season2PondsData as any)?.ponds?.map((p: any) => (
                          <MenuItem key={p.id} value={p.id}>
                            {p.name}
                          </MenuItem>
                        ))}
                    ...
                    renderValue={selected => (
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                        {selected.map(val => (
                          <Chip
                            key={val}
                            label={metricOptions.find(m => m.id === val)?.name || val}
                            size='small'
                          />
                        ))}
                      </Box>
              ...
              <Grid item xs={12}>
                <Button
                  ...
                  disabled={isProcessing || selectedMetrics.length === 0}
                >
          ...
          <CardContent>
            ...
            >
              {selectedMetrics.map(mid => (
                <Tab key={mid} label={metricOptions.find(m => m.id === mid)?.name || mid} />
              ))}
            </Tabs>
            <Box sx={{ height: 400 }}>
              <ResponsiveContainer width='100%' height='100%'>
                <ComposedChart
                  data={formatChartData(comparisonData.metrics?.[selectedMetrics[activeTab]])}
                >
                  ...
                  <Line
                    type='monotone'
                    dataKey='pondA'
                    name={getPondName(mode === 'current' ? selectedPondA : selectedPondA_H)}
                    stroke='#2563EB'
                  />
                  <Line
                    type='monotone'
                    dataKey='pondB'
                    name={getPondName(mode === 'current' ? selectedPondB : selectedPondB_H)}
                    stroke='#10B981'
                  />
                </ComposedChart>
              </ResponsiveContainer>
