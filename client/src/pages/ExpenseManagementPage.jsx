...
const ExpenseManagementPage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [expRes, pondsRes] = await Promise.all([
        getExpenses(selectedSeason?.id),
        getPonds()
      ]);
      setExpenses(expRes);
        ...
        <form onSubmit={handleSubmit}>
          <DialogContent>
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <TextField select fullWidth label="Category" required value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
                  {CATEGORIES.map(c => <MenuItem key={c} value={c}>{c}</MenuItem>)}
                </TextField>
              </Grid>
              <Grid item xs={12}>
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <DatePicker label="Date" value={formData.date} onChange={date => setFormData({...formData, date})} renderInput={(params) => <TextField {...params} fullWidth required />} />
                </LocalizationProvider>
              </Grid>
              <Grid item xs={12}>
                <TextField fullWidth label="Amount ($)" type="number" inputProps={{ step: "0.01" }} required value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} />
              </Grid>
              <Grid item xs={12}>
                <TextField select fullWidth label="Pond (Optional)" value={formData.pondId} onChange={e => setFormData({...formData, pondId: e.target.value})} helperText="Leave blank if this is a general/season-wide expense.">
                  <MenuItem value=""><em>None (Season-wide)</em></MenuItem>
